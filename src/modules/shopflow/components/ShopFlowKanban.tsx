...
export function ShopFlowKanban({ orders }: ShopFlowKanbanProps) {
  ...
  return (
    <div className={`flex gap-4 overflow-x-auto pb-4 ${isMobile ? 'px-2' : ''}`}>
      {lanes.map((lane) => {
        const laneOrders = ordersByStatus[lane.key] || [];
        return (
          <div
            key={lane.key}
            className={`flex-shrink-0 bg-card/30 rounded-lg border border-border/50 p-4 shadow-sm ${
              isMobile ? 'w-72' : 'w-80'
            }`}
          >
            ...
            <ScrollArea className={isMobile ? "h-[400px]" : "h-[calc(100vh-300px)]"}>
